...
def average_model_weights(models):
    avg_model = copy.deepcopy(models[0][2])
    state_dicts = [m[2].state_dict() for m in models]
    with torch.no_grad():
        for key in avg_model.state_dict():
            avg_model.state_dict()[key].copy_(sum(sd[key] for sd in state_dicts) / len(state_dicts))
    return avg_model
'''
...
def average_model_weights_weighted(models, weights):
    """Average weights of given models using specified weights.

    Args:
        models: list of (dist, code, model)
        weights: list of floats (same length), should sum to 1

    Returns:
        avg_model: a new model with averaged weights
    """
    ...
    # Deepcopy one model as the base
    avg_model = copy.deepcopy(models[0][2])  # Use model from (dist, code, model)
    avg_state_dict = avg_model.state_dict()

    # Get all model state_dicts
    model_state_dicts = [m.state_dict() for (_, _, m) in models]
    ...
    return avg_model
'''
def average_model_weights_ema(models, ema_decay):
    """
    ...
    Returns:
        avg_model: a new model with EMA-averaged weights
    """
    avg_model = copy.deepcopy(models[0][2])  # base
    avg_state_dict = avg_model.state_dict()

    # Extract state_dicts of all models
    model_state_dicts = [m.state_dict() for (_, _, m) in models]

    for key in avg_state_dict:
        # Start with the first model
        ema_val = model_state_dicts[0][key].clone()
        for i in range(1, len(model_state_dicts)):
            ema_val = ema_decay * ema_val + (1 - ema_decay) * model_state_dicts[i][key]
        avg_state_dict[key] = ema_val
...
def average_model_weights_weighted(models, weights):
    ...
    # Extract all model state_dicts from their `.module` (inner quba)
...
'''
def average_ddp_models(models):
    """Average the parameters of multiple DDP-wrapped models and return a new unwrapped state_dict (without 'module.')."""
    avg_state_dict = {}
    num_models = len(models)
    
    # Get raw (unwrapped) state dict from first model
    keys = models[0].module.state_dict().keys()

    for key in keys:
        avg_state_dict[key] = sum(m.module.state_dict()[key] for m in models) / num_models
...
def load_and_rebuild_model(code, checkpoint_path, device_id, c_dist, n_node_features, n_edge_features, msg_net_size, n_iters, use_ddp=True):
    """
    Rebuilds a model from code and loads saved weights.

    Args:
        code: panqec.Code instance for the desired distance/code structure.
        checkpoint_path: path to the saved .pth model checkpoint.
        device_id: GPU id (int) for DDP.
        c_dist: code distance (e.g., 6, 12, 18).
        n_node_features, n_edge_features: architecture dimensions.
        msg_net_size: hidden size of message passing network.
        n_iters: number of MP iterations.
        use_ddp: if True, wraps with DDP. Set False for local evaluation.

    Returns:
        model: a (DDP-wrapped) model loaded with checkpoint weights.
    """
    # 1. Create base model with correct code structure
    model = init_model(
        code=code,
        c_dist=c_dist,
        n_node_features=n_node_features,
    ...
    # 2. Load weights (should match this architecture)
    state_dict = torch.load(checkpoint_path, map_location=f"cuda:{device_id}")
    model.load_state_dict(state_dict)

    # 3. Move to correct device
    model = model.to(f"cuda:{device_id}")

    # 4. Wrap in DDP if needed
    if use_ddp:
...
def evaluate_and_save_model(
    ...
    stop_training, prev_best_ler, no_improve_epochs, min_test_err_rate, min_lerz, patience
):
    ...
    if rank == 0:
        ...
        if lerz == 0:
            ...
            print(f"[Early Stopping] lerz=0. Stopping at epoch {epoch}.")
    ...
    #return min_test_err_rate, min_lerz, prev_best_ler, stop_training.item() == 1
...
def evaluate_model_only(model, code, testloader, test_sampler, device, rank, use_amp, epoch, optimizer, scheduler, train_loss, start_time, prefix=""):
    ...
    if rank == 0:
        #print(f"[{prefix}] Epoch {epoch}: LER_X={lerx:.6f}, LER_Z={lerz:.6f}, LER_tot={ler_tot:.6f}, TestLoss={test_loss:.6f}")
...
model_configs = [
    ...
    #{"c_dist": 10, "n_node_features": 32, "n_edge_features": 64, "msg_net_size": 256, "n_iters": 40, "sub_len_train_set": 50000, "test_len": 3000},
    #{"c_dist": 12, "n_node_features": 32, "n_edge_features": 32, "msg_net_size": 256, "n_iters": 50, "sub_len_train_set": 80000, "test_len": 4000},
    #{"c_dist": 18, "n_node_features": 64, "n_edge_features": 32, "msg_net_size": 128, "n_iters": 65, "sub_len_train_set": 100000, "test_len": 5000}
]
...
#len_train_set = s_train_data_start
...
#optimizer = optim.AdamW(model_start.parameters(), lr=lr_max, weight_decay=weight_decay)#this includes all weights and biases of layers like Linear, GRU, LayerNorm, etc.
#scheduler = optim.lr_scheduler.StepLR(optimizer,step_size=40,gamma=0.5)


# Add before the training loop:
#le_rates = torch.zeros(E_total, 5)
...
for epoch in range(E_total):
    ...
    if epoch < E_prime:
        ...
        if (epoch + 1 - E_prime) % lambda_step == 0 or epoch == E_m-1:
            ...
            #avg_model = average_model_weights_ema(models, ema_decay=ema_factor)
    ...
    # === After Warm-up: Copy averaged model to all domain models and to model_start ===
        ...
        stop_flag = False
        #min_test_err_rate, min_lerz, prev_best_ler, stop_flag = evaluate_and_save_model(
