...
from quba_sagu.functions import quba, collate, compute_accuracy, logical_error_rate, \
...
parser.add_argument('--len_train_set', type=int, default=5, help='Length of training set')
parser.add_argument('--len_test_set', type=int, default=2, help='Length of test set')
...
fnamenew = f"models/ours_final/{code.name}_d{c_dist}_{error_model_name}_{n_iters}_{n_node_features}_{n_edge_features}_" \
...
if rank == 0:
    print("code.N =", code.N)
    print("syndrome_error_volume.shape:", val_data.shape)
    print("generate_syndrome_error_volume", val_data)
    print("len(generate_syndrome_error_volume):", len(val_data))
